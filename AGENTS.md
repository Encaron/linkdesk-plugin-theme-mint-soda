...
> 插件身份的唯一来源 = `plugin.json` 顶层的 `pluginId`（本仓：`theme-mint-soda`）。当前版本 `1.0.3`。

## 1. 这是什么

薄荷苏打——四套薄荷系亮色配色（薄荷冰露 / 海盐薄荷 / 青柠薄荷 / 薄荷苏打），Claymorphism 软萌风。
...
配方在 `themes/mint-soda.json`（一个文件四套 colorway）。
